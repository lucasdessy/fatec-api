...
export default class Account {
  ...
  public async login(): Promise<any> {
    ...
    await page.goto(Network.ROUTES.LOGIN);
    await page.waitForNetworkIdle();
    await page.type("#vSIS_USUARIOID", this.username);
    await page.type("#vSIS_USUARIOSENHA", this.password);
    await Promise.all([page.click(".Button"), page.waitForNavigation()]);
    this.state = Account.STATES.LOGGED;
    this.cookie = (await page.cookies()).toString();
    browser.close();
  }
          ...
          return Network.scrap({
            ...
            scrapper: ($) => {
              ...
              const calendar = months.map((month, monthIndex) => {
                ...
                events = events.map((i: any, e: { data: any; }) => e.data).get();
                return {
                  events: events.reduce((_events: any[], event: string | string[]) => {
                    event = event.trim();
                    if (event.length) {
                      event = event.split("-");
                      event = {
  ...
  public getSchoolGrade(): Promise<any> {
    ...
    return prom.then(() => {
      return Network.scrap({
        ...
        scrapper: ($) => {
          const semesters = $("#TABLE1 table [valign=TOP]").map((i: number, el: any) => {
            const item: any = {};
            item.number = i + 1;
            item.disciplines = $(el).find(":scope div").map((_: any, div: any) => {
              const $discipline = $(div);
              const data = $discipline.find("tr td").map((__i: any, td: any) => {
                const $td = $(td);
                const str = $td.text().trim();
                if (str.indexOf("NF:") > -1) {
                  return [
                    $td.contents().not($td.children()).text(),
                  ].concat($td.find("b").map((___i: any, b: any) => $(b).text().trim()).get());
                }
                return str;
              }).get();
    ...
    return prom.then(() => {
      return Network.scrap({
        ...
        scrapper: ($) => {
          ...
          this.student.setPartialGrades(data.map((line: { [x: string]: any[]; }) => {
            let disciplineState;
            ...
            return {
              discipline,
              evaluations: line["Avaliacoes"].map((evaluation: { [x: string]: string; Notas: any[]; }) => {
                return new Evaluation({
                  ...
                  grades: evaluation.Notas.map((grade: { [x: string]: any; }) => {
                    return {
        ...
        scrapper: ($$) => {
          ...
          return Network.scrap({
            ...
            scrapper: ($) => {
              ...
              this.student.setEnrolledDisciplines(data.map((line: { [x: string]: any; }) => {
                const scheduleDiscipline = scheduleData.filter((d: { [x: string]: any; }) => {
